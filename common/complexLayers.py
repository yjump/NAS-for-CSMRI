...
from torch.nn import Conv2d, Linear, BatchNorm1d, BatchNorm2d, LeakyReLU, InstanceNorm2d, ZeroPad2d
from common.complexFunctions import complex_relu, complex_max_pool2d, complex_leakyrelu, complex_upsample
...
class ComplexLeakyRelu(Module):
    ...
    def forward(self, input_r, input_i):
        return complex_leakyrelu(input_r, input_i)


class ComplexModRelu(Module):

    def forward(self, input_r, input_i):
        inp = torch.cat([input_i, input_r], dim=1)
        mod = torch.clamp((inp ** 2).sum(dim=1).sqrt(), min=1e-5).unsqueeze(dim=1)
        return input_r * torch.relu(1. - 0.5 / mod), input_i * torch.relu(1. - 0.5 / mod)
...
class ComplexUpsample(Module):
    ...
    def forward(self, input_r, input_i):
        return complex_upsample(input_r, input_i, factor=self.scale_factor)
...
class ComplexZeroPad2d(Module):
    ...
    def forward(self, input_r, input_i):
        #        assert(input_r.size() == input_i.size())
        return self.pad_r(input_r), self.pad_i(input_i)
...
class NaiveComplexInstanceNorm2d(Module):
    '''
    Naive approach to complex instance norm, perform batch norm independently on real and imaginary part.
    '''

    def __init__(self, num_features, eps=1e-5, momentum=0.1):
        super(NaiveComplexInstanceNorm2d, self).__init__()
        self.in_r = InstanceNorm2d(num_features, eps, momentum)
        self.in_i = InstanceNorm2d(num_features, eps, momentum)

    def forward(self, input_r, input_i):
        return self.in_r(input_r), self.in_i(input_i)
